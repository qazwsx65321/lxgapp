...
@interface XDDatePickerView ()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation XDDatePickerView
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView{
    return self.isHour ? self.dataArray.count : 3;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (component) {
        case 0: { // 年
            ...
            if ((year_integerValue.integerValue < time_integerValue) && self.isCurrentAuto) {
        ...
        case 1: { // 月
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if ((month_value.integerValue < [self.timeArr[component] integerValue]) && self.isCurrentAuto) {
        ...
        case 2: { // 日
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    // 如果选择的日小于当前日，就刷新到当前日
                    if ((day_value.integerValue < [self.timeArr[component] integerValue]) && self.isCurrentAuto) {
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    if ([self.day integerValue] > [self.timeArr[2] integerValue]) {
                        ...
                        // 如果选择的时小于当前时，就刷新到当前时
                        if (([self.hourArr[row%[self.dataArray[component] count]] integerValue] < [self.timeArr[3] integerValue]) && self.isCurrentAuto) {
                            [pickerView selectRow:[self.dataArray[component] indexOfObject:self.timeArr[component]] inComponent:component animated:YES];
                            // 如果选择的时大于当前时，就直接赋值
                        } else {
                            self.hour = self.hourArr[row%[self.dataArray[component] count]];
...
- (void)setIsCurrentAuto:(BOOL)isCurrentAuto {
    _isCurrentAuto = isCurrentAuto;
}
