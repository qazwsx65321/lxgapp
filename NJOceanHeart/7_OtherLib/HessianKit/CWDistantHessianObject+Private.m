...
-(NSString*)methodNameFromInvocation:(NSInvocation*)invocation;
{
  ...
  if (!methodName) {
    ...
    unsigned long realParamCount = [[invocation methodSignature] numberOfArguments] - 2;
    if (realParamCount > 0) {
      methodName = [methodName stringByAppendingFormat:@"__%ld", realParamCount];
    }
...
-(NSData*)sendRequestWithPostData:(NSData*)postData;
{
  ...
  if (requestError) {
    responseData = nil;
      ...
      responseData = nil;
        NSLog(@"HTTP error %zd",[returnResponse statusCode]);
    }
  } else {
    responseData = nil;
  }
  ...
  if (code == 'r') {
    ...
    int minor = [unarchiver readChar];
      ...
      if ([unarchiver readChar] != 'z') {
          NSError *error = [NSError errorWithDomain:@"unarchivererror" code:1000 userInfo:@{@"ErrorDescription":@"Did not find reply terminator z"}];
          return error;
      }
      return object;
    } else {
     NSError *error = [NSError errorWithDomain:@"unarchivererror" code:1001 userInfo:@{@"ErrorDescription":[NSString stringWithFormat:@"Unsupported version %d.%d", major, minor]}];
    return error;
    }
  } else  if (code == 'f') {
    [self readHeaderFromUnarchiver:unarchiver];
      NSDictionary* failMap = [unarchiver readMap];
      NSError *error = [NSError errorWithDomain:@"unarchivererror" code:[[failMap objectForKey:@"code"] integerValue] userInfo:@{@"ErrorDescription":[failMap objectForKey:@"description"]}];
      
      return error;
  } else {
    NSError *error = [NSError errorWithDomain:@"unarchivererror" code:1003  userInfo:@{@"ErrorDescription":@"Unknown response data"}];
    return error;
  }
  return nil;
}
...
-(void)setReturnValue:(id)value invocation:(NSInvocation*)invocation;
{
  ...
  if (strcmp(type, @encode(void)) == 0) {
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
      ...
      NSError *error = [NSError errorWithDomain:@"NotSupported" code:1004 userInfo:@{@"":[NSString stringWithFormat:@"Unsupported type %s", type]}];
      NSLog(@"%@",error);
  }
  if (isInvalidClass) {
      NSError *error = [NSError errorWithDomain:@"NotSupported" code:1004 userInfo:@{@"":[NSString stringWithFormat:@"Invalid type %@",NSStringFromClass([value class])]}];
      NSLog(@"%@",error);
  }
}
