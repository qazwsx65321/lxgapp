...
#define DefaultSegmentHeight 64
...
- (XHSegmentControl *)segmentControl
{
    if (!_segmentControl)
    {
        CGFloat y = 0;//!self.navigationController?20:64;    DefaultSegmentHeight
        _segmentControl = [[XHSegmentControl alloc] initWithFrame:CGRectMake(0, y, [UIScreen mainScreen].bounds.size.width, _NavHeight ? _NavHeight : DefaultSegmentHeight)];
        _segmentControl.delegate = self;
    }
    return _segmentControl;
}

- (void)setNavHeight:(float)NavHeight {
    _NavHeight = NavHeight;
    
    _segmentControl.height = NavHeight;
}
...
-(void)setSegmentLineColor:(UIColor *)segmentLineColor{
    ...
    self.segmentControl.lineColor = segmentLineColor;
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    if (!scrollView.isDecelerating) {
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:@"MainStoreScorllEvent" object:@NO];
...
{
    [[NSNotificationCenter defaultCenter]postNotificationName:@"MainStoreScorllEvent" object:@YES];
