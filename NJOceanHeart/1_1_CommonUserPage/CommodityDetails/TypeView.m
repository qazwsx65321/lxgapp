...
@property (nonatomic,weak) UIButton * p_selectBtn;

@property (nonatomic,strong) NSArray * p_guiArr;
...
{
    if (self.p_selectBtn != btn) {
        btn.selected = YES;
        [btn setBackgroundColor:BackgroundColors(1)];
        self.p_selectBtn.selected = NO;
        [self.p_selectBtn setBackgroundColor:RGB(240, 240, 240)];
        self.p_selectBtn = btn;
    }
    
    int tag = (int)btn.tag-100;
    NSString *str = self.p_guiArr[tag];
    [self.delegate btnFromView: self index:(int)btn.tag-100 andGuiGe:str];
