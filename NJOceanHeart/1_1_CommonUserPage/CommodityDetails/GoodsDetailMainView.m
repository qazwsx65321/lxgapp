...
-(void)initBottomView
{
    bottomView = [[BottomView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-47, self.frame.size.width, 47)];
...
-(void)initDetailViewImgArr:(NSArray *)imgarr andWebArr:(NSArray *)webarr;
{
    ...
    goodsDetail = [[GoodsDetailView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, bottomView.frame.origin.y) andImageArr:imgarr];
//    goodsDetail.contentInset = UIEdgeInsetsMake(64, 0, 0, 0);
    [self addSubview:goodsDetail];
    goodsDetail.backgroundColor = [UIColor blueColor];
    //宝贝详情内容
