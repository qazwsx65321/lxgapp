...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        label.font = [UIFont fontWithName:Heiti_Light size:15];
        self.p_label = label;
        ...
        accebutton.titleLabel.font = [UIFont systemFontOfSize:14];
        [accebutton setImage:[UIImage imageNamed:@"more01"] forState:0];
        [self addSubview:accebutton];
        [accebutton addTarget:self action:@selector(moreClick) forControlEvents:UIControlEventTouchUpInside];
...
-(void)moreClick{
    ...
    [self.delegate headviewClickMoreMethod];
...
-(void)layoutSubviews{
    ...
    self.p_label.y = 10;
    self.p_label.centerY = self.height/2;
