//
//  Ocean_IntelligentQuestionAnswerVC.m
//  NJOceanHeart
//
...
@interface Ocean_IntelligentQuestionAnswerVC ()<UITableViewDelegate,UITableViewDataSource,Ocean_ButtonCellDelegate>
...
@implementation Ocean_IntelligentQuestionAnswerVC
...
- (Ocean_ZhiNengQuestionFrame *)cellframes {
    if (!_cellframes) {
        _cellframes = [[Ocean_ZhiNengQuestionFrame alloc] init];
    }
    return _cellframes;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        Ocean_ZhiNengQuestionCell *cell = [Ocean_ZhiNengQuestionCell cellWithTableView:tableView];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (self.infoDic.count) {
            return self.cellframes.cellhight;
        }else {
            return 0;
        }
    }
...
- (void)didInquiry:(Ocean_ButtonCell *)cell {
    if (self.keyText.text.length > 0) {
        ...
        [self usingToolsPayWithID:self.model.m_sid withPrice:self.model.m_price withSuccess:^{
            ...
            self.cellframes = [[Ocean_ZhiNengQuestionFrame alloc] init];
            
            [MBProgressHUD showActivityMessageInWindow:@"加载中..."];
            
            [Ocean_DataTools GetZhiNengAnswerWithQuestion:self.keyText.text withBlock:^(id data) {
                
                self.cellframes.dic = data;
                
                self.infoDic = self.cellframes.dic;
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [MBProgressHUD hideHUD];
                    [MBProgressHUD showInfoMessage:@"暂无相关回答信息!"];
                    [self.tableView reloadData];
